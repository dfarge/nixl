...
# 3FS Docker Build Wrapper Script
# This script creates a Docker container and builds 3FS completely from scratch
# Based on: https://github.com/deepseek-ai/3FS/issues/78

set -e

echo "=== 3FS Docker Build Wrapper ==="
echo "This script will create a Docker container and build 3FS from scratch"
echo
...
# Download the playground script if it doesn't exist
if [ ! -f "playground.sh" ]; then
    echo "Downloading playground.sh..."
...
# Copy binaries to a persistent location
mkdir -p /build-output
cp -r /tmp/3fs/build/bin/* /build-output/

echo "Binaries copied to /build-output for persistence"
echo "Available binaries:"
ls -lh /build-output/ | awk '{print $9, $5}' | column -t
...
echo "✅ Binaries copied to /build-output"
echo
echo "You can now use the 3FS binaries for setting up your distributed filesystem!"
EOF

# Make the build script executable
chmod +x build-3fs-complete.sh

echo "=== Creating Docker container ==="
# Run the playground script to create container
sudo bash playground.sh

echo "=== Copying build script to container ==="
# Copy our complete build script to the container
docker cp build-3fs-complete.sh build-3fs-playground:/tmp/

echo "=== Starting automated build ==="
# Execute the build script inside the container
docker exec -it build-3fs-playground bash /tmp/build-3fs-complete.sh

echo "=== Copying built binaries to host ==="
# Create output directory on host
mkdir -p 3fs-binaries
# Copy binaries from container to host
docker cp build-3fs-playground:/build-output/. 3fs-binaries/

echo
echo "=== Complete! ==="
echo "3FS has been built successfully!"
echo "Binaries are available in: $(pwd)/3fs-binaries/"
echo
ls -lh 3fs-binaries/
echo
echo "Container 'build-3fs-playground' is still running if you need to access it:"
echo "  docker exec -it build-3fs-playground bash"
echo
echo "To stop the container:"
echo "  docker stop build-3fs-playground" 